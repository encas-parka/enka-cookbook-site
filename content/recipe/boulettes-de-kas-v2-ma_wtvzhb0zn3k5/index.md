---
title: Boulettes de kasha vegan
date: '2026-03-29T11:18:35.183Z'
draft: true
typeR: plat
categories:
  - Bouchées salées
regime:
  - vegan
  - sans-gluten
  - sans-lactose
plate: 6
quantite_desc: null
check: true
cuisson: true
serveHot: true
description: null
region: null
saison: []
id: boulettes-de-kas-v2-ma_wtvzhb0zn3k5
materiel: []
ingredients:
  - uuid: 9f35oz
    name: Sarrasin
    originalQuantity: 240
    originalUnit: gr.
    normalizedQuantity: 240
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: sec
  - uuid: rtavky
    name: Oignon
    originalQuantity: 2
    originalUnit: unité
    normalizedQuantity: 240
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: legumes
  - uuid: vs2dfm
    name: Ail en poudre
    originalQuantity: 5
    originalUnit: gr.
    normalizedQuantity: 5
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: epices
  - uuid: ique18
    name: Paprika fumé
    originalQuantity: 10
    originalUnit: gr.
    normalizedQuantity: 10
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: epices
  - uuid: jjrwlj
    name: Sauce soja
    originalQuantity: 3
    originalUnit: c. à soupe
    normalizedQuantity: 45
    normalizedUnit: ml
    comment: ''
    allergens:
      - Soja
    type: epices
  - uuid: 5c1b20
    name: Moutarde
    originalQuantity: 1
    originalUnit: c. à soupe
    normalizedQuantity: 15
    normalizedUnit: ml
    comment: ''
    allergens:
      - Moutarde
    type: epices
  - uuid: mck2wl
    name: Concentré de tomate
    originalQuantity: 1
    originalUnit: c. à soupe
    normalizedQuantity: 15
    normalizedUnit: ml
    comment: ''
    allergens: []
    type: epices
  - uuid: 2juq3x
    name: Sel
    originalQuantity: 0
    originalUnit: gr.
    normalizedQuantity: 0
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: epices
  - uuid: 2pps90
    name: Poivre
    originalQuantity: 2
    originalUnit: gr.
    normalizedQuantity: 2
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: epices
  - uuid: c70jxb
    name: huile de tournesol
    originalQuantity: 100
    originalUnit: ml
    normalizedQuantity: 100
    normalizedUnit: ml
    comment: ''
    allergens: []
    type: lof
  - uuid: he3u48
    name: Cumin moulu
    originalQuantity: 3
    originalUnit: gr.
    normalizedQuantity: 3
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: epices
  - uuid: 7jceco
    name: Curry
    originalQuantity: 1
    originalUnit: c. à café
    normalizedQuantity: 2
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: epices
preparation: |-
  Faire griller le sarrasin à sec jusqu'à ce qu'il fonce (sans noircir). Ajouter de l'eau (2x le volume de sarrasin, voire plus si besoin), couvrir pour cuire a l'étouffée. Une fois cuit, laisser refroidir en l'étalant dans des gastros.

  Pendant ce temps, faire dorer-fondre les oignons émincés puis ajouter les épices, laisser s'infuser 2 min et couper le feu. Déglacer avec la sauce soja, mélanger puis ajouter moutarde et concentré de tomates.

  Mettre dans un gros saladier, incorporer le sarrasin grillé refroidi, assaissonner à votre goût.

  Façonner en boulettes, huiler au pinceau, mettre au four à 200°C (environ 30 min, retourner autour de la mi cuisson).
prepAlt: []
astuces: []
preparation24h: null
permissionWrite:
  - 6931ecdccd2c8d8d15e5
auteur: Marion
createdAt: '2026-03-29T11:18:34.650+00:00'
updatedAt: '2026-03-29T11:18:34.650+00:00'
createdBy: 6931ecdccd2c8d8d15e5
teams: []
status: public
rootRecipeId: boulettes-de-kasha_0tffvh1bmxwr
versionLabel: v2 - Marion
---

