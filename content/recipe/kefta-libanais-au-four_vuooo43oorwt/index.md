---
title: Kefta libanais au four
date: '2026-08-19T08:00:35.085Z'
draft: false
typeR: plat
categories:
  - Plat au Four
regime:
  - sans-gluten
  - sans-lactose
plate: 4
quantite_desc: null
check: true
cuisson: true
serveHot: true
description: null
region: Libanais
saison: []
id: kefta-libanais-au-four_vuooo43oorwt
materiel: []
ingredients:
  - uuid: gnbf8z
    name: Viande hachée (boeuf)
    originalQuantity: 400
    originalUnit: gr.
    normalizedQuantity: 400
    normalizedUnit: gr.
    comment: marche aussi avec poulet ou agneau
    allergens:
      - Viande
    type: animaux
  - uuid: doxujz
    name: Oignon rouge
    originalQuantity: 60
    originalUnit: gr.
    normalizedQuantity: 60
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: legumes
  - uuid: hgfa4e
    name: Persil frais
    originalQuantity: 0.1
    originalUnit: bottes
    normalizedQuantity: 0.1
    normalizedUnit: bottes
    comment: ''
    allergens: []
    type: epices
  - uuid: y16lzq
    name: Menthe fraîche
    originalQuantity: 0.02
    originalUnit: bottes
    normalizedQuantity: 0.02
    normalizedUnit: bottes
    comment: ''
    allergens: []
    type: epices
  - uuid: fetrpe
    name: Coriandre fraîche
    originalQuantity: 0.1
    originalUnit: bottes
    normalizedQuantity: 0.1
    normalizedUnit: bottes
    comment: ''
    allergens: []
    type: epices
  - uuid: bidsmu
    name: Paprika
    originalQuantity: 1
    originalUnit: c. à soupe
    normalizedQuantity: 6
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: epices
  - uuid: e4mjd1
    name: Jus de citron
    originalQuantity: 2
    originalUnit: c. à soupe
    normalizedQuantity: 30
    normalizedUnit: ml
    comment: ''
    allergens: []
    type: sucres
  - uuid: 163u4z
    name: Olives Vertes Dénoyautées
    originalQuantity: 40
    originalUnit: gr.
    normalizedQuantity: 40
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: legumes
  - uuid: dc8855
    name: Cumin
    originalQuantity: 1
    originalUnit: pincées
    normalizedQuantity: 0.4
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: epices
  - uuid: dtvho0
    name: Brochette en bois
    originalQuantity: 8
    originalUnit: unité
    normalizedQuantity: 8
    normalizedUnit: unité
    comment: ''
    allergens: []
    type: autres
  - uuid: atccqu
    name: Huile végétale
    originalQuantity: 4
    originalUnit: c. à soupe
    normalizedQuantity: 60
    normalizedUnit: ml
    comment: ''
    allergens: []
    type: lof
  - uuid: 2juq3x
    name: Sel
    originalQuantity: 1
    originalUnit: au goût
    normalizedQuantity: 1
    normalizedUnit: au goût
    comment: ''
    allergens: []
    type: epices
  - uuid: 2pps90
    name: Poivre
    originalQuantity: 1
    originalUnit: au goût
    normalizedQuantity: 1
    normalizedUnit: au goût
    comment: ''
    allergens: []
    type: epices
preparation: "- Commencer par hacher l’oignon, la coriandre fraiche, le persil ainsi que la menthe les verser dans un saladier avec la viande hachée  puis ajouter toutes les épices\n- Mixer rapidement les olives et les ajouter à la préparation avec le jus de citron\n- Bien malaxer et réserver au frais afin que la farce se raffermit\n- Former des boudins et les piquer dans les pics en bois\_\n- Badigeonner chaque brochette d’un peu de beurre fondu ou d’huile d’olive et les passer au four pendant 10 minutes à 180 °/ 200°."
prepAlt: []
astuces: []
preparation24h: null
permissionWrite:
  - 6931ecdccd2c8d8d15e5
auteur: Marion
createdAt: '2026-07-21T16:13:33.268+00:00'
updatedAt: '2026-08-19T08:00:34.812+00:00'
createdBy: 6931ecdccd2c8d8d15e5
teams: []
status: public
rootRecipeId: null
versionLabel: null
---

