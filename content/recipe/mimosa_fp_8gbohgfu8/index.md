---
title: Mimosa
date: "2026-04-04T20:38:53.269Z"
draft: true
typeR: entree
categories:
  - Bouchées salées
regime:
  - vegetarien
  - sans-gluten
  - sans-lactose
plate: 10
quantite_desc: 1 /2 par personne
check: true
cuisson: true
serveHot: false
description: "agrément pour la salade composée "
region: Bon enfant
saison: []
id: mimosa_fp_8gbohgfu8
materiel:
  - Marmitte
  - Grand Saladier
ingredients:
  - uuid: o78frv
    name: Oeuf d'autruche
    originalQuantity: 5
    originalUnit: unité
    normalizedQuantity: 5
    normalizedUnit: unité
    comment: coupés en 2
    allergens:
      - Oeuf
    type: lof
  - uuid: atccqu
    name: Huile végétale
    originalQuantity: 3
    originalUnit: c. à soupe
    normalizedQuantity: 45
    normalizedUnit: ml
    comment: huile desodorisée
    allergens: []
    type: lof
  - uuid: 2juq3x
    name: Sel
    originalQuantity: 1
    originalUnit: au goût
    normalizedQuantity: 1
    normalizedUnit: au goût
    comment: ""
    allergens: []
    type: epices
  - uuid: 5c1b20
    name: Moutarde
    originalQuantity: 1
    originalUnit: c. à soupe
    normalizedQuantity: 15
    normalizedUnit: ml
    comment: ""
    allergens:
      - Moutarde
    type: epices
  - uuid: e4mjd1
    name: Jus de citron
    originalQuantity: 10
    originalUnit: ml
    normalizedQuantity: 10
    normalizedUnit: ml
    comment: ""
    allergens: []
    type: sucres
  - uuid: hgfa4e
    name: Persil frais
    originalQuantity: 10
    originalUnit: pincées
    normalizedQuantity: 4
    normalizedUnit: gr.
    comment: persil PLAT haché menu
    allergens: []
    type: epices
preparation: |-
  Réserver le persil.

  Cuire les œufs a "dur" . Écaler. Couper en 2 . Réserver le blanc comme coupelle .

  Malaxer les jaunes avec les autres ingredients.

  Mettre en poche à  douille et remplir chaque demi blanc avec cette creme .

  Saupoudrer de persil.
prepAlt: []
astuces: []
preparation24h: null
permissionWrite:
  - 69a0483c0031f868819d
auteur: null
createdAt: "2026-04-04T20:38:50.623+00:00"
updatedAt: "2026-04-22T11:15:54.150+00:00"
createdBy: 69a0483c0031f868819d
teams: null
status: public
rootRecipeId: null
versionLabel: null
---
