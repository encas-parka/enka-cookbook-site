---
title: Salade micuite aubergine pois chiche
date: '2026-06-15T21:39:33.837Z'
draft: false
typeR: entree
categories:
  - Salade
regime:
  - vegan
  - sans-gluten
  - sans-lactose
plate: 40
quantite_desc: une portion = 225gr
check: true
cuisson: true
serveHot: false
description: null
region: null
saison:
  - ete
  - automne
id: salade-micuite-aubergi_ouq92dry-vuv
materiel: []
ingredients:
  - uuid: 7sk3a5
    name: Aubergine
    originalQuantity: 5
    originalUnit: kg
    normalizedQuantity: 5000
    normalizedUnit: gr.
    comment: coupé en dé taille pois chiche environ
    allergens: []
    type: legumes
  - uuid: egkb0a
    name: Tomates cerises
    originalQuantity: 1.25
    originalUnit: kg
    normalizedQuantity: 1250
    normalizedUnit: gr.
    comment: coupées en 2 ou 4
    allergens: []
    type: legumes
  - uuid: wwl5vn
    name: Pois chiche cuit
    originalQuantity: 4
    originalUnit: kg
    normalizedQuantity: 4000
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: sec
  - uuid: owsv5m
    name: échalote
    originalQuantity: 300
    originalUnit: gr.
    normalizedQuantity: 300
    normalizedUnit: gr.
    comment: émincées
    allergens: []
    type: legumes
  - uuid: he3u48
    name: Cumin moulu
    originalQuantity: 50
    originalUnit: gr.
    normalizedQuantity: 50
    normalizedUnit: gr.
    comment: au gout
    allergens: []
    type: epices
  - uuid: bwkn34
    name: Citron (jus)
    originalQuantity: 250
    originalUnit: gr.
    normalizedQuantity: 250
    normalizedUnit: gr.
    comment: au gout
    allergens: []
    type: legumes
  - uuid: 2juq3x
    name: Sel
    originalQuantity: 20
    originalUnit: gr.
    normalizedQuantity: 20
    normalizedUnit: gr.
    comment: au gout
    allergens: []
    type: epices
  - uuid: h3c8al
    name: huile d'olive
    originalQuantity: 250
    originalUnit: gr.
    normalizedQuantity: 250
    normalizedUnit: gr.
    comment: au gout
    allergens: []
    type: lof
  - uuid: c70jxb
    name: huile de tournesol
    originalQuantity: 100
    originalUnit: gr.
    normalizedQuantity: 100
    normalizedUnit: gr.
    comment: pour la cuisson
    allergens: []
    type: lof
  - uuid: hgfa4e
    name: Persil frais
    originalQuantity: 2
    originalUnit: bottes
    normalizedQuantity: 2
    normalizedUnit: bottes
    comment: émincé
    allergens: []
    type: epices
preparation: |-
  Couper les aubergine en dés et faire revenir dans une grande poêle pour qu'elles soient cuites, de préférence légèrement grillées et tenues (si c'est arrangeant c'est aussi possible au four)

  Une fois qu'elles ont a peu près refroidies rajouter les pois chiches cuits égouttés, rincés, les tomates cerises coupés en 2 ou 4, les échalotes émincées, le persil émincés, du jus de citron, de l'huile d'olive, du sel, du cumin en gouttant!

  Réserver au frais et servir environ 225gr par personne
prepAlt: []
astuces: []
preparation24h: null
permissionWrite:
  - 696fb1cc000eb33adee7
auteur: Miki
createdAt: '2026-04-25T13:31:41.687+00:00'
updatedAt: '2026-06-15T21:39:32.611+00:00'
createdBy: 696fb1cc000eb33adee7
teams: []
status: public
rootRecipeId: null
versionLabel: null
---

