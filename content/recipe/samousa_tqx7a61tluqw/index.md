---
title: Samousa
date: "2026-04-04T21:33:41.845Z"
draft: true
typeR: entree
categories:
  - Bouchées salées
regime:
  - vegan
  - sans-lactose
plate: 10
quantite_desc: 2 /personne
check: true
cuisson: true
serveHot: true
description: " rouleaux croustillants fourrés parfumés "
region: Indienne
saison: []
id: samousa_tqx7a61tluqw
materiel:
  - Four
ingredients:
  - uuid: 18dy4e
    name: Pomme de terre
    originalQuantity: 600
    originalUnit: gr.
    normalizedQuantity: 600
    normalizedUnit: gr.
    comment: cuites
    allergens: []
    type: legumes
  - uuid: 0lpad0
    name: Poivron
    originalQuantity: 100
    originalUnit: gr.
    normalizedQuantity: 100
    normalizedUnit: gr.
    comment: "vert , haché menu "
    allergens: []
    type: legumes
  - uuid: jyius1
    name: petit pois
    originalQuantity: 50
    originalUnit: gr.
    normalizedQuantity: 50
    normalizedUnit: gr.
    comment: "surgelés "
    allergens: []
    type: legumes
  - uuid: up1ydm
    name: feuille de pâte à  samoussa
    originalQuantity: 20
    originalUnit: unité
    normalizedQuantity: 20
    normalizedUnit: unité
    comment: ""
    allergens:
      - Gluten
    type: sec
  - uuid: e4mjd1
    name: Jus de citron
    originalQuantity: 15
    originalUnit: ml
    normalizedQuantity: 15
    normalizedUnit: ml
    comment: ""
    allergens: []
    type: sucres
  - uuid: rtavky
    name: Oignon
    originalQuantity: 2
    originalUnit: unité
    normalizedQuantity: 240
    normalizedUnit: gr.
    comment: coupé menu
    allergens: []
    type: legumes
  - uuid: fetrpe
    name: Coriandre fraîche
    originalQuantity: 3
    originalUnit: unité
    normalizedQuantity: 3
    normalizedUnit: unité
    comment: ciselée
    allergens: []
    type: epices
  - uuid: he3u48
    name: Cumin moulu
    originalQuantity: 0
    originalUnit: gr.
    normalizedQuantity: 0
    normalizedUnit: gr.
    comment: ""
    allergens: []
    type: epices
  - uuid: nf05n7
    name: Coriandre en poudre
    originalQuantity: 0
    originalUnit: gr.
    normalizedQuantity: 0
    normalizedUnit: gr.
    comment: ""
    allergens: []
    type: epices
  - uuid: 0d1fzw
    name: Gingembre
    originalQuantity: 1
    originalUnit: c. à soupe
    normalizedQuantity: 15
    normalizedUnit: ml
    comment: "râpé "
    allergens: []
    type: legumes
  - uuid: ljin96
    name: garam  massala
    originalQuantity: 0
    originalUnit: gr.
    normalizedQuantity: 0
    normalizedUnit: gr.
    comment: ""
    allergens:
      - Moutarde
    type: epices
  - uuid: wr5x7u
    name: asa foetida
    originalQuantity: 0
    originalUnit: gr.
    normalizedQuantity: 0
    normalizedUnit: gr.
    comment: ""
    allergens:
      - Céleri
    type: epices
preparation: |-
  Reserver la coriandre et les petits pois .

  Dorer les oignons dans un peu de huile à  friture desodorisée. Malaxer avec tout le reste et cuire a feu doux avec les poudres suivantes :  cumin 1c. à  dessert  + garam massala 1 c à  dessert + asa foetida 1/2 c à  dessert + coriandre 1/2 c à  dessert.

  Garnir les feuilles de pâte a l aide de une poche a douille. Compléter avec des petits pois et de la coriandre ciselée. Rouler .Dorer à  four moyen.
prepAlt: []
astuces: []
preparation24h: null
permissionWrite:
  - 69a0483c0031f868819d
auteur: null
createdAt: "2026-04-04T21:33:41.362+00:00"
updatedAt: "2026-04-22T11:15:54.150+00:00"
createdBy: 69a0483c0031f868819d
teams: null
status: public
rootRecipeId: null
versionLabel: null
---
