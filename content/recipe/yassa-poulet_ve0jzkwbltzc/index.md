---
title: Yassa poulet
date: '2026-04-12T18:16:40.496Z'
draft: true
typeR: plat
categories:
  - Plat en Sauce
regime:
  - sans-gluten
  - sans-lactose
plate: 10
quantite_desc: 1 bol par personne
check: true
cuisson: true
serveHot: true
description: ragoût relevé à accompagné de riz nature
region: 'Sénégal '
saison: []
id: yassa-poulet_ve0jzkwbltzc
materiel: []
ingredients:
  - uuid: 5cg0yu
    name: Citron jaune
    originalQuantity: 4
    originalUnit: unité
    normalizedQuantity: 400
    normalizedUnit: gr.
    comment: 3 citrons pressés et chair du quatrieme
    allergens: []
    type: legumes
  - uuid: rtavky
    name: Oignon
    originalQuantity: 1.2
    originalUnit: kg
    normalizedQuantity: 1200
    normalizedUnit: gr.
    comment: gros morceaux
    allergens: []
    type: legumes
  - uuid: u2m28w
    name: Ail
    originalQuantity: 5
    originalUnit: gousse·s
    normalizedQuantity: 32.5
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: legumes
  - uuid: rl1e9p
    name: Poivron (vert)
    originalQuantity: 50
    originalUnit: gr.
    normalizedQuantity: 50
    normalizedUnit: gr.
    comment: coupé fin
    allergens: []
    type: legumes
  - uuid: 5c1b20
    name: Moutarde
    originalQuantity: 20
    originalUnit: gr.
    normalizedQuantity: 20
    normalizedUnit: gr.
    comment: ''
    allergens:
      - Moutarde
    type: epices
  - uuid: vs2dfm
    name: Ail en poudre
    originalQuantity: 0
    originalUnit: gr.
    normalizedQuantity: 0
    normalizedUnit: gr.
    comment: ''
    allergens: []
    type: epices
  - uuid: 1ej217
    name: huile de friture
    originalQuantity: 1
    originalUnit: au goût
    normalizedQuantity: 1
    normalizedUnit: au goût
    comment: ''
    allergens: []
    type: lof
  - uuid: a3widy
    name: Emincés de poulet (volaille et gibier)
    originalQuantity: 150
    originalUnit: gr.
    normalizedQuantity: 150
    normalizedUnit: gr.
    comment: le moins possible de blanc de poulet .
    allergens:
      - Viande
    type: animaux
  - uuid: j84olg
    name: Feuille de laurier
    originalQuantity: 1
    originalUnit: unité
    normalizedQuantity: 1
    normalizedUnit: unité
    comment: ''
    allergens: []
    type: epices
preparation: |-
  Faire dorer dans l’huile les oignons et le poulet 

  Ajouter les ingredients restants et faire mijoter. Ajouter de l’eau pour une sauce généreuse.
prepAlt: []
astuces: []
preparation24h: null
permissionWrite:
  - 69a0483c0031f868819d
auteur: null
createdAt: '2026-04-12T18:16:39.803+00:00'
updatedAt: '2026-04-12T18:16:39.803+00:00'
createdBy: 69a0483c0031f868819d
teams: null
status: public
rootRecipeId: null
versionLabel: null
---

