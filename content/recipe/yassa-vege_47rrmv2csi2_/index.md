---
title: Yassa végé
date: "2026-04-04T20:52:40.548Z"
draft: true
typeR: plat
categories:
  - Plat en Sauce
regime:
  - vegan
  - sans-gluten
  - sans-lactose
plate: 10
quantite_desc: 1 bol moyen /personne. accompagné idéalement de riz blanc
check: true
cuisson: true
serveHot: true
description: chunks de soja dans une sauce vigoureuse
region: "Sénégalaise "
saison: []
id: yassa-vege_47rrmv2csi2_
materiel: []
ingredients:
  - uuid: pbydyr
    name: Chunks (protéine de soja)
    originalQuantity: 1000
    originalUnit: gr.
    normalizedQuantity: 1000
    normalizedUnit: gr.
    comment: marinees et cuites
    allergens:
      - Soja
    type: autres
  - uuid: e4mjd1
    name: Jus de citron
    originalQuantity: 4
    originalUnit: unité
    normalizedQuantity: 4
    normalizedUnit: unité
    comment: ""
    allergens: []
    type: sucres
  - uuid: rtavky
    name: Oignon
    originalQuantity: 10
    originalUnit: unité
    normalizedQuantity: 1200
    normalizedUnit: gr.
    comment: coupé moyen
    allergens: []
    type: legumes
  - uuid: 5c1b20
    name: Moutarde
    originalQuantity: 1
    originalUnit: au goût
    normalizedQuantity: 1
    normalizedUnit: au goût
    comment: ""
    allergens:
      - Moutarde
    type: epices
  - uuid: u2m28w
    name: Ail
    originalQuantity: 5
    originalUnit: gousse·s
    normalizedQuantity: 32.5
    normalizedUnit: gr.
    comment: ""
    allergens: []
    type: legumes
  - uuid: 0lpad0
    name: Poivron
    originalQuantity: 50
    originalUnit: gr.
    normalizedQuantity: 50
    normalizedUnit: gr.
    comment: coupé  menu menu.
    allergens: []
    type: legumes
  - uuid: atccqu
    name: Huile végétale
    originalQuantity: 1
    originalUnit: au goût
    normalizedQuantity: 1
    normalizedUnit: au goût
    comment: désodorisée friture
    allergens: []
    type: lof
  - uuid: 8d1cfa5bf2ae
    name: Huile d'arachide
    originalQuantity: 1
    originalUnit: au goût
    normalizedQuantity: 1
    normalizedUnit: au goût
    comment: pimentée, pour ceux-celles qui aiment
    allergens:
      - Arachides
    type: lof
preparation: |-
  Mariner les chunks dans citron+moutarde+poivron.

  Dorer les oignons dans l huile .

  Tout melanger et cuire en marmite , chunks fondants .

  On peut mettre de l huile pimentée  à  dispo.
prepAlt: []
astuces: []
preparation24h: null
permissionWrite:
  - 69a0483c0031f868819d
auteur: null
createdAt: "2026-04-04T20:52:39.940+00:00"
updatedAt: "2026-04-22T11:15:54.150+00:00"
createdBy: 69a0483c0031f868819d
teams: null
status: public
rootRecipeId: null
versionLabel: null
---
