...
migrate((app) => {
  ...
  unmarshal({
    ...
    "updateRule": "@request.auth.id != \"\" && (ownerUser = @request.auth.id || teamId.members ~ @request.auth.id)",
    "viewRule": "@request.auth.id != \"\" && (ownerUser = @request.auth.id || teamId.members ~ @request.auth.id)"
  }, collection)
