...
export function mergeByKey<T>(key: keyof T & string) {
  return (local: T[], remote: T[]): T[] => {
    ...
    for (const item of remote ?? [])
      map.set((item as Record<string, unknown>)[key], item);
...
export function createSyncCollection<T extends AwDoc>(options: {
  ...
  const onRealtimeEvent = syncOptions?.onRealtimeEvent;
  ...
  async function initialFetch(fetchOptions?: AwFetchOptions): Promise<void> {
    ...
    const syncKey = fetchOptions?.scopeKey
      ? `${collectionId}:${fetchOptions.scopeKey}`
      : collectionId;
  ...
  function subscribeToCollection(
    ...
    const handler = async (response: any) => {
      ...
      if (isDelete) {
        ...
        console.log(
          `[aw-sync] realtime DELETE ${String(collectionName)}/${payload.$id}`,
        );
        onRealtimeEvent?.(payload, "delete");
        ...
        onRealtimeEvent?.(payload, isCreate ? "create" : "update");
      }
    };
  ...
  async function unsubscribe(
    subRefOrId: SubscriptionRef | string,
  ): Promise<void> {
  ...
  async function update(id: string, data: Partial<T>): Promise<T> {
    ...
    if (!current) {
      throw new Error(
        `[aw-sync] Record ${id} not found in Dexie (${String(collectionName)})`,
      );
    }
    ...
    try {
      ...
      console.warn(
        `[aw-sync] update ROLLBACK ${String(collectionName)}/${id}`,
        err.message,
      );
      throw err;
    }
  ...
  async function remove(id: string): Promise<void> {
    ...
    try {
      ...
      console.warn(
        `[aw-sync] remove ROLLBACK ${String(collectionName)}/${id}`,
        err.message,
      );
      throw err;
    }
