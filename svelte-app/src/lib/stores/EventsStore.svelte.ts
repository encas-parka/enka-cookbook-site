...
export class EventsStore {
	...
	#bridge: BridgeResult<Main> = bridgeToMap<Main>(
		() => db.events.toArray()
	);
	...
	getUpcomingEventsForUser(): EnrichedEvent[] {
		...
		return events.filter((event) => {
			...
			if (
				event.teamsId?.some((teamId) =>
					this.#userTeams.includes(teamId),
				)
			)
				return true;
	...
	async initialize(): Promise<void> {
		...
		if (this.#initPromise) {
			console.log(
				'[EventsStore] Initialisation déjà en cours, attente...',
			);
	...
	async fetchEvent(eventId: string): Promise<EnrichedEvent | null> {
		try {
			...
			console.error(
				`[EventsStore] Erreur lors du fetch de ${eventId}:`,
				err,
			);
			return null;
		}
	...
	canEditEvent(eventId: string): boolean {
		...
		if (
			event.teams?.some((teamId) => this.#userTeams.includes(teamId))
		)
			return true;
	...
	#serializeUpdateData(
		...
		if (data.dateStart !== undefined)
			serialized.dateStart = data.dateStart;
		if (data.dateEnd !== undefined) serialized.dateEnd = data.dateEnd;
		if (data.allDates !== undefined)
			serialized.allDates = data.allDates;
		...
		if (data.contributors !== undefined) {
			serialized.contributors = data.contributors.map((c) =>
				JSON.stringify(c),
			);
		}
	...
	async updateEventStatus(
		...
		try {
			...
			console.log(
				`[EventsStore] Statut mis à jour: ${eventId} -> ${status}`,
			);
		} catch (err) {
			console.error(
				`[EventsStore] Erreur mise à jour statut ${eventId}:`,
				err,
			);
			throw err;
		}
	...
	getContributorStatus(eventId: string): string {
		const event = this.#enrichedMap.get(eventId);
		if (!event) return '';
		const user = event.contributors.filter(
			(c) => c.id === this.#userId,
		);
	...
	async removeContributor(
		...
		try {
			...
			console.error(
				`[EventsStore] Erreur suppression contributeur:`,
				err,
			);
			throw err;
		}
	...
	async inviteParticipants(
		...
		try {
			...
			if (
				...
				console.log(
					`[EventsStore] Aucun participant à inviter`,
				);
				return event;
			}
			...
			const result = await inviteParticipantsToEvent(
				eventId,
				event.name,
				{
					teamIds,
			...
			console.error(
				`[EventsStore] Erreur invitation participants:`,
				err,
			);
			throw err;
		}
	...
	async removeTeam(
		...
		try {
			...
			if (!updatedEvent)
				throw new Error("Impossible de recharger l'événement");
	...
	async deleteMeal(
		...
		try {
			...
			console.error(
				`[EventsStore] Erreur suppression meal:`,
				err,
			);
			throw err;
		}
	...
	async addRecipeToEvent(
		...
		if (undatedMeal) {
			...
			if (
				undatedMeal.recipes.some((r) => r.recipeUuid === recipeUuid)
			) {
				throw new Error(
					'Recette déjà mise de côté pour cet événement',
				);
			}
	...
	async updateTodoStatus(
		...
		try {
			...
			console.error(
				`[EventsStore] Erreur updateTodoStatus:`,
				err,
			);
			throw err;
		}
	...
	async toggleTodoAssignment(
		...
		try {
			...
			console.error(
				`[EventsStore] Erreur toggleTodoAssignment:`,
				err,
			);
			throw err;
		}
	...
	async deleteTodo(
		...
		try {
			...
			console.error(
				`[EventsStore] Erreur suppression todo:`,
				err,
			);
			throw err;
		}
