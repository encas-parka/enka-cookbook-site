...
class RecipesStore {
  ...
  async #loadIndexFromDataJson(): Promise<void> {
    ...
    if (updatedIds.length > 0) {
      await db.recipeData.bulkDelete(updatedIds.map((id) => `detail:${id}`));
    }
    ...
    await db.transaction("rw", db.recipeData, db.syncMeta, async () => {
      ...
      const rows = Array.from(this.#hugoRecipes.values()).map((entry) => ({
        key: `idx:${entry.$id}`,
  ...
  async forceReloadAllRecipes(): Promise<void> {
    ...
    try {
      ...
      const addedCount = appwriteRecipes.filter(
        (r) => r.status !== "deleted",
      ).length;
      const deletedCount = appwriteRecipes.filter(
        (r) => r.status === "deleted",
      ).length;
  ...
  #rebuildFuseIndexIfNeeded(): void {
    const currentSize = this.#recipesIndex.size;
    if (currentSize === this.#lastFuseSize && this.#fuseTitle) return;
    this.#lastFuseSize = currentSize;
    ...
    try {
      ...
      if (localRecipe) {
        return (
          ...
          Boolean(localRecipe.permissionWrite?.includes(globalState.userId)) ||
          Boolean(
  ...
  async getRecipesByUuidsBulk(
    ...
    try {
      const fetchPromises = stillMissing.map(
        async (
          uuid,
        ): Promise<{ uuid: string; recipe: RecipeForDisplay } | null> => {
          // 3a. Try Hugo
          try {
            const response = await fetch(`/recipe/${uuid}/recipe.json`);
            if (response.ok) {
          ...
          if (globalState.userId) {
            const recipe = await this.#loadDetailFromAppwriteRemote(uuid);
  ...
  async #loadDetailFromDexie(uuid: string): Promise<RecipeForDisplay | null> {
